...
describe('WalletHome', () => {
  it('renders fixture identity, balances, a vault position, and a non-duplicated total', async () => {
    ...
    expect(screen.getByText('WBTC')).toBeOnTheScreen();
    expect(screen.getByText('0.0100 WBTC')).toBeOnTheScreen();
  ...
  it('renders wallet actions and reports the selected action', async () => {
    ...
    await screen.findByText('$4,045.00');
  ...
  it('hides and reveals every financial amount', async () => {
    ...
    expect(screen.queryByText('0.0100 WBTC')).not.toBeOnTheScreen();
    ...
    expect(screen.getAllByLabelText('Hidden amount')).toHaveLength(9);
    ...
    await waitFor(() => expect(screen.getByText('$4,045.00')).toBeOnTheScreen());
  });
  ...
  it('offers retry after a provider failure', async () => {
    ...
    expect(await screen.findByText('$4,045.00')).toBeOnTheScreen();
  });
  ...
  it('reacts to provider updates and renders an empty portfolio', async () => {
    ...
    expect(screen.queryByText('$4,045.00')).not.toBeOnTheScreen();
  });
