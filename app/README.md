...
`EXPO_PUBLIC_ENS_CLAIMS_ENABLED=0` keeps the separate **Request ENS name** diagnostic hidden. Onboarding uses the same live API and issuer flow without that diagnostic flag: create or reopen the passkey wallet, review and activate its Kernel if undeployed, choose and check an available label, then authorize the claim with the primary passkey. The claim ID is saved locally and checked again after restarts. Onboarding finishes only after the issuer reports confirmation and current ENS ownership and resolution both point to the Kernel. Existing mock-era profiles reopen their original wallet and go through the real claim step. The launcher and Wallet display the ENS name only while those checks continue to pass; otherwise they show the address.
