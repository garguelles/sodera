...
`pnpm verify:device-webauthn` verifies the committed public fixture at `app/scripts/fixtures/pra185-device-assertion.json`. It checks the authenticator data, client-data semantics, normalized DER signature, P-256 public key, and encoded envelope agree before submitting that genuine assertion envelope to released Sepolia validator `0x7ab16Ff354AcB328452F1D445b3Ddee9a91e9e69` with only its public-key storage supplied by an `eth_call` state override. The exact operation returned `validationData = 0`; changing the final nibble of the operation hash while reusing the assertion returned `validationData = 1`. This consumes the genuine provider signature through the released verifier without deploying an account or moving assets. The fixture contains no raw credential ID, user handle, account label, biometric data, or private material.
...
The minimal implementation is one Android-only Expo local module that owns Credential Manager calls, activity/foreground/cancellation state, exception mapping, and raw response transfer. JavaScript owns explicit operation confirmation, supplies fresh request JSON, and rejects a native result if Sodera is not active when the result crosses the ceremony acceptance boundary. The existing Kernel WebAuthn bridge owns all assertion validation and validator-envelope construction. No Turnkey API, remotely managed signer, Android Keystore signing API, private-key export, backend signer, or Play Integrity decision belongs in this path.
