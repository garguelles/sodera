...
- ZeroDev-hosted infrastructure is the required target for Ethereum Sepolia, pending verification. Arc may use compatible alternative infrastructure while retaining Kernel and the Primary Passkey authority.
- Verify Ethereum Sepolia's released ZeroDev WebAuthn validator/Kernel execution path, ZeroDev sponsorship, executable Uniswap testnet swaps and liquidity, and live Graph data availability before treating dependent implementation specs as ready.
- Pin the exact released ZeroDev SDK, WebAuthn key package, Kernel, EntryPoint, factory, WebAuthn validator, signature envelope, and deployment configuration. Verify source and deployed bytecode/configuration provenance rather than relying on defaults or support lists. Do not use an unreleased raw-P256 validator or a remotely managed wallet signer.
...
- Include quotes, slippage protection, approval review, confirmation, and required WebAuthn user verification.
...
- Use The Graph for vault and position data. Deposits and withdrawals require transaction review, simulation, confirmation, and required WebAuthn user verification.
...
- [ADR-0011](adr/0011-ensv2-user-subnames.md) selects the tokenized, user-owned ENSv2 subname architecture for that deferred work on the current Sepolia deployment. The ETHOnline deployment research is historical and does not establish ownership on the current hierarchy.
- **Subsequent scope update (2026-09-26):** The owner elected to proceed with real ENS onboarding and free, one-year renewable user subnames. The implementation decisions and deployment gates are in [the ENS onboarding plan](plans/ens-onboarding-and-renewal.md). The fixture description above records the existing app behavior, not the target behavior.
- **Hackathon demo decision (2026-09-27):** Repeated presentations use the deployed Railway API and issuer worker to create a fresh passkey-controlled testnet wallet and unused ENS label each time. The simplest phone reset is `adb shell pm clear xyz.sodera.app`, which clears app metadata but not provider passkeys, on-chain wallets/names, or the Railway claim ledger. ENS claims have no per-day cap for this hackathon; one active name per Kernel/label and short-window challenge limits remain. This does not change the separately agreed ZeroDev sponsorship policy or the controls required before a broader production release.
- **ENS renewal timing (2026-09-27):** User subnames expire one year after registration. The free-renewal design is retained, but implementing renewal is deferred until **after the hackathon**; no renewal role or endpoint is live for this build.
...
- Recovery must work after loss of the original phone. Google-free GrapheneOS support is deferred for the hackathon. A recovery credential accessible only on the lost phone, or synchronized rediscovery of the primary passkey alone, is insufficient evidence for the independent recovery flow.
- Verify provider behavior on supported Android and compatibility with the selected Kernel recovery mechanism. Record GrapheneOS compatibility separately if tested; it is not hackathon acceptance scope.
- The recovery demonstration must use the recovery passkey to authorize a replacement primary passkey and revoke the lost primary passkey while retaining the same smart account.
- Fresh onboarding, including after reinstalling or moving to a new phone, offers Create Wallet or Recover Wallet.
- Recover Wallet uses the previously enrolled recovery passkey to restore control of the existing smart account and username, replacing and revoking the lost primary passkey instead of creating another account. The installation then holds only the recovered wallet.

## Signing Authorization

- Use a user-controlled platform WebAuthn passkey as Kernel's direct primary authority through ZeroDev's released WebAuthn validator. The passkey must not merely authenticate to Turnkey or another remotely managed wallet signer.
- Require explicit transaction review and confirmation before the WebAuthn ceremony. Do not permit silent background signing.
- Require WebAuthn user verification and pin the credential used for the account. Verify the actual Android credential-provider behavior, RP/domain association, challenge binding, cancellation, lockout, and credential-loss behavior on supported test devices.
- Do not claim StrongBox, hardware-backed TEE, device binding, non-exportability, or a particular biometric/device-credential policy unless separately evidenced for the selected provider and device.
- Google-free GrapheneOS support is deferred for the hackathon. Play Integrity remains prohibited from controlling wallet ownership or asset access.
...
- Use one shared allowance of 10 sponsored operations per wallet per day, covering onboarding, ENS registration, recovery enrollment, primary-passkey replacement, sends, and swaps.
...
- Detailed sponsorship eligibility for lost-device recovery and primary-passkey replacement; these consume the shared allowance, with no separate reserved budget.
